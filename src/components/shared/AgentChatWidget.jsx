...
/**
 * Reusable floating agent chat widget.
 * Props:
 *  - agentName: string (agent file name without .json)
 *  - triggerIcon: ReactNode (icon shown on the FAB)
 *  - title: string (chat panel header label)
 *  - placeholder: string (input placeholder)
 *  - emptyMessage: string (shown when conversation starts)
 *  - fabClassName: string (extra classes for the FAB button)
 */
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-24 right-5 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all active:scale-95 ${fabClassName}`}
          ...
          <div className="pointer-events-auto w-full max-w-sm h-[520px] bg-white rounded-3xl shadow-2xl border border-border flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              ...
              {conversation && messages.length === 0 && (
                <p className="text-xs text-muted-foreground text-center pt-8">{emptyMessage}</p>
              )}
              {messages.map((msg, i) => (
